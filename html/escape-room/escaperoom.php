...
<head>
	...
	<script>
			...
			if (msg.startsWith('sound')) {	//שינוי מנגינת רקע
				...
				if (!listOfWrongCode[data[0]].includes(data[1])) {	//בודק אם הקוד כבר הוזן
					...
					if (!isTest) {
						...
						httpRequest.send("pass=" + pass + "&riddle=" + data[0] + "&code=" + data[1] + "&time=" + time);
					}
				}
				else {
					printWorning(0, 1);
				...
				if (msg == 90) {
					...
					if (!isTest) {
						...
						httpRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
						httpRequest.send("pass=" + pass + "&score=" + score + "&time=" + time);
					}
				...
				if (!isTest) {
					...
					httpRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
					httpRequest.send("pass=" + pass + "&cookie=" + msg + "&time=" + time);
				}
			}
		...
		//זיהוי לחיצה על מקשים
		function keyboardResponse(event) {
			if (event.keyCode === 13) {		//enter
				let inp = document.getElementById('passInp');
				pass = inp.value.toLowerCase();
				...
				if (pass !== "") {
					...
					httpRequest.onreadystatechange = function () {
						if (this.readyState === 4 && this.status === 200) {
							...
							if (response === 'n' || response === 't') {
								setCookie("solved", 0);
								document.getElementById('passDiv').style.display = 'none';
								document.getElementById('loadfilsediv').style.display = 'block';
								isTest = response == 't';
								loadfilse();
							}
							else if (response === 'w') {
								...
								score = responseArr[2];
								document.cookie = `solved=${responseArr[3]}`;
		...
		function updateProgressTime() {	//מעדכן את ההתקדמות של הזמן
			...
			httpRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			httpRequest.send("pass=" + pass + "&time=" + time);
		}
		...
		function listOfClueUsedToNum() {	//עדכון מעקב אחרי רמזים
			...
			httpRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			httpRequest.send("pass=" + pass + "&clue=" + num + "&time=" + time);
		}
